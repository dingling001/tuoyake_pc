  ...
  methods: {
    ...
    getWeekD(id) {
      this.$api.getWeekD(id).then(res => {
        this.first = res.data;
        this.banners = res.data.default_img;
        this.link = window.origin + "/detail?id=" + this.first.article_id;
        setTimeout(() => {
          this.initS();
        }, 500);
      });
